...
Minimum viable implementation to prove the concept: Claude Code and Codex running in two terminals, holding a synchronous conversation via `lesche` with a git-backed transcript.

## Goal

Two agents, two terminals, one tunnel. The Claude Code session sends a message with `lesche send` and blocks. The Codex session receives it with `lesche await`. Codex sends a reply; Claude's original `send` returns that reply. Repeat for several turns. Close the tunnel. Inspect the git history in `~/.lesche/workspace/` and see every message.
...
**In**
- Single binary `lesche` (Go).
- Auto-spawned daemon listening on `~/.lesche/sock`.
- In-memory tunnel state with turn enforcement.
- Single project, single tunnel type (two-party sync).
- Git-backed transcript in `~/.lesche/workspace/`.
...
- Resumable blocking (`lesche resume`). Default timeout returns an error; agent retries by calling `send`/`await` again if desired.
...
```
lesche CLI  ──unix socket──▶  lesche daemon (goroutines + in-memory state)
                                         │
                                         ▼
                               ~/.lesche/workspace/ (git repo)
                               └── tunnels/<sid>/
                                   ├── SESSION.md
                                   └── NNN-<from>.md
```
...
- Auto-spawn on first CLI call; `~/.lesche/sock` as the unix socket.
...
- No idle timeout in MVP — run until `lesche stop` or SIGINT.

### Workspace

- `~/.lesche/workspace/` initialized as a git repo on first daemon start (if not already).
...
| Command | Behavior |
|---|---|
| `lesche register --name <name>` | Spawn daemon if needed. Record `{name, pid, started_at}` in daemon state. Exit 0 with `name` printed. Re-registering same `name` from same pid is idempotent; from different pid returns error. |
| `lesche agents` | Print one line per registered agent: `<name>  <pid>  <started_at>`. |
| `lesche tunnel <peer>` | Create a tunnel with `<peer>`. `<peer>` must be registered. Returns `sid`. State initialized with `turn=caller`. Does not block — the tunnel is ready for the caller to `send`. If peer is not registered, exit non-zero with "peer not registered". |
| `lesche send <sid> "msg" [--timeout 300]` | Append message, commit, block until peer sends next message or timeout. If not caller's turn, exit with "not_your_turn". Returns peer's reply on stdout. |
| `lesche await <sid> [--timeout 300]` | Block until a message arrives on this tunnel or timeout. If caller's turn (they owe a send, not await), exit with "not_your_turn". Returns the message on stdout. |
| `lesche close <sid>` | Mark tunnel closed. If peer is blocked on `await` or `send`, their call returns exit code 3 with "peer_closed". |
...
1. Project scaffold, CLI arg parsing (Go + `spf13/cobra` or stdlib `flag`). Binary `lesche`.
2. Unix socket server/client: define a simple length-prefixed JSON request/response protocol. One request = one response.
3. Daemon process: auto-spawn via `exec.Command` + double-fork-ish detach (or just `setsid` on Linux/macOS). PID file at `~/.lesche/pid`.
...
```
$ lesche register --name claude
claude

$ lesche tunnel codex
sid=01HX9Z

$ lesche send 01HX9Z "hello codex, can you hear me?"
# blocks …
# returns: "yes, I hear you. what's the question?"

$ lesche send 01HX9Z "are you running gpt-5?"
# blocks …
...
$ lesche send 01HX9Z "yes, I hear you. what's the question?"
# blocks …
# returns: "are you running gpt-5?"

$ lesche send 01HX9Z "yes, gpt-5 via codex CLI."
# blocks …
...
1. Both agents register without error; `lesche agents` lists both.
...
6. `lesche close` terminates the tunnel; peer's blocked call returns exit 3.
...
Consolidated from two lesche tunnel exchanges between Claude Opus 4.7 and GPT-5 (via Codex) on 2026-04-17 (`tunnels/2417d8c0c877/` and `tunnels/d14651711e66/` in the workspace log), reviewed with the user afterward.
...
3. **Session discovery.** `lesche sessions` (list open tunnels involving caller) and `lesche await-any` (block on the first incoming message from any tunnel). Ranked P0 from direct experience — without these, a receiving agent has no way to discover an inbound tunnel and is forced to grep the workspace filesystem.
...
- **Write queue → SQLite** at `~/.lesche/queue.db` (WAL). Narrow purpose only: persist messages between "client ack" and "git commit" so a daemon crash does not lose acknowledged-but-uncommitted messages.
...
- **Resumable blocking** (`lesche resume <sid>`). Solves the tool-call timeout wall for long LLM turns. Claude ranks this P0; Codex ranks phase 2. Agreed tiebreak: first observed in-field failure promotes it.
