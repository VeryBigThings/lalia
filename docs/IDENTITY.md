# Lalia — Identity model (post-MVP design)
...
The `lalia register` command auto-populates these fields from the
...
```
lalia nickname <nick> <address>        # assign
lalia nickname <nick>                  # show what it resolves to, plus current status
lalia nickname                         # list all nicknames
...
```
lalia nickname reviewer  claude@obolos:web
lalia nickname scratch   claude@obolos:quant
lalia nickname pair      codex@obolos:main
```

Then, in any command that takes an address:

```
lalia tunnel reviewer
```
...
**Stable binding (default)**: `lalia nickname <nick> <address>`
...
registered. Reassign with: lalia nickname reviewer <new-address>
```

**Role binding** (opt-in): `lalia nickname --follow <nick> <address>`
...
`lalia nickname <nick>` shows the binding mode and the current
resolution:
...
3. Change lookup functions to accept the resolution grammar in section
   "Address resolution."
4. Update `lalia agents` output to show fully qualified form.
5. Add `lalia nickname` subcommand.
6. Update `lalia protocol` to document the new addressing rules.
