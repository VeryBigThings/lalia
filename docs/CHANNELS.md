...
authoritative in `help.go` / `lalia protocol`.
...
lalia ask  <peer> "msg" [--timeout N] tell + block for next inbound from peer
lalia read <peer>       [--timeout N] block until next inbound (0 = non-blocking)
lalia peek <peer>                     non-destructive inspect of pending

# Room
lalia room_create <name> [--desc s]
lalia join     <room>
lalia leave    <room>
lalia rooms
lalia participants <room>
lalia post <room> "msg"               async broadcast
lalia read <room>  [--timeout N]      consume pending (0 = non-blocking)
lalia peek <room>                     non-destructive inspect

# Housekeeping (unchanged in shape, may change args)
lalia register | agents | renew | history <peer|room> | stop | protocol
```

Seven communication verbs total: `tell`, `ask`, `post`, `read`,
`peek`, `join`, `leave`. Plus room/registration management.

### English ↔ command mapping (ships in `lalia protocol` output)
...
- `session` → replaced by `peek` + a new `lalia channels` listing.
...
1. **Implicit channel creation.** `tell X` on a peer you have
   ...
   we want an explicit `lalia open <peer>` gate? Recommendation:
...
is user-visible. `lalia tunnel` going away (even as a deprecated
