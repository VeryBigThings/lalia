...
// cmdNickname implements: lesche nickname [<nick> [<address>]] [-d <nick>] [--follow]
func cmdNickname(args []string) {
	...
	if args[0] == "-d" {
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: lesche nickname -d <nick>")
			os.Exit(1)
		}
		nick := args[1]
		nicknames, err := loadNicknames()
	...
	// assign: lesche nickname [--follow] <nick> <address>
	follow := false
	remaining := args
	...
	if len(remaining) < 2 {
		fmt.Fprintln(os.Stderr, "usage: lesche nickname [--follow] <nick> <address>")
		os.Exit(1)
	}
	nick = remaining[0]
	address := remaining[1]
